...
def get_args():
    ...
    parser.add_argument('--add_sam', type=int, default=0, help='if value = 0 ,dont use SAM,if value = 1 adding SAM')
    ...
    parser.add_argument('--mom_lamda', type=float, default=0, help='momen step lenth')
    parser.add_argument('--beta', type=float, default=0, help='MAUpdate')
    parser.add_argument('--calls', type=str, default="defaut", help='hyperparameter for stable')
...
def main():
    ...
    if args.alg_name == 'FedDyn':  
        ...
        print('FedMA')
        [fed_mdls_sel_FedALL, trn_perf_sel_FedALL, tst_perf_sel_FedALL,
        fed_mdls_all_FedALL, trn_perf_all_FedALL,
        tst_perf_all_FedALL] = train_FedMA(data_obj=data_obj, act_prob=act_prob,
                                         learning_rate=learning_rate, batch_size=batch_size,
                                     epoch=epoch, com_amount=com_amount, print_per=print_per, weight_decay=weight_decay,
                                     model_func=model_func, init_model=init_model, save_period=save_period,
                                     lr_decay_per_round=lr_decay_per_round, args=args, seed=seed_val)
        tst_perf_alg = tst_perf_all_FedALL.copy()
    
    elif alg_name == 'FedACG':
        print('FedACG')
        [fed_mdls_sel_FedALL, trn_perf_sel_FedALL, tst_perf_sel_FedALL,
        fed_mdls_all_FedALL, trn_perf_all_FedALL,
        tst_perf_all_FedALL] = train_FedACG(data_obj=data_obj, act_prob=act_prob,
                                         learning_rate=learning_rate, batch_size=batch_size,
                                     epoch=epoch, com_amount=com_amount, print_per=print_per, weight_decay=weight_decay,
                                     model_func=model_func, init_model=init_model, save_period=save_period,
                                     lr_decay_per_round=lr_decay_per_round, args=args, seed=seed_val)
