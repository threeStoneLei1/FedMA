...
def Layermom_reg_set_client_from_params(mdl, clnt_params_list, selected_clnts, momen_agg_sim):
    dict_param = copy.deepcopy(dict(mdl.named_parameters()))
    idx = 0
    for name, _ in mdl.named_parameters():
        dict_param[name].data.zero_()
    for name, param in mdl.named_parameters():
        weights = param.data
        length = len(weights.reshape(-1))
        for clnt in selected_clnts:
            step_size = momen_agg_sim[clnt][name] * ( 1 / len(selected_clnts))
            dict_param[name].data.add_(torch.tensor(clnt_params_list[clnt][idx:idx + length].reshape(weights.shape) * step_size))
        idx += length

    mdl.load_state_dict(dict_param)
    return mdl

def Layermom_norm_set_client_from_params(mdl, clnt_params_list, selected_clnts, momen_agg_weight):
    dict_param = copy.deepcopy(dict(mdl.named_parameters()))
    idx = 0
    for name, _ in mdl.named_parameters():
        dict_param[name].data.zero_()
    for name, param in mdl.named_parameters():
        ...
        length = len(weights.reshape(-1))
        for clnt in selected_clnts:
            step_size = momen_agg_weight[clnt][name]
            dict_param[name].data.add_(torch.tensor(clnt_params_list[clnt][idx:idx + length].reshape(weights.shape) * step_size))
        idx += length
...
def train_model(model, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name, args):
    n_trn = trn_x.shape[0]
    ...
    for e in range(epoch):
        ...
        if (e + 1) % print_per == 0:
            # raise ValueError(f"No parameters found for layer with name")
...
def train_FedMA_model(model, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name,
                       args, avg_model, momen_model, loss_conpare):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size,
                              shuffle=True)
    # loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
    loss_fn = torch.nn.CrossEntropyLoss().to(device)
    loss_norm = torch.nn.MSELoss().to(device)
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    model.train()
    model = model.to(device)
    beta = args.beta
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_trn / batch_size))):
            ...
            loss_momen_reg = 0
            if loss_conpare and (e !=  0 or i !=0):
                avg_model_params = torch.cat([param.view(-1) for param in avg_model.parameters()])
                model_params = torch.cat([param.view(-1) for param in model.parameters()])
                momen_params = torch.cat([param.view(-1) for param in momen_model.parameters()])
                updated_params = model_params - avg_model_params
                loss_momen_reg = torch.norm(updated_params - momen_params)
                loss += beta * loss_momen_reg
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
        ...
        if (e + 1) % print_per == 0:
            ...
            print("Epoch %3d, Training Accuracy: %.4f, Loss: %.4f" % (e + 1, acc_trn, loss_trn))
            model.train()
    # print(f"{e:{epoch}, i:{int(np.ceil(n_trn / batch_size))},sum:{epoch * int(np.ceil(n_trn / batch_size))}}")
...
def train_FedACG_model(model, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name,
                       args, avg_model, loss_conpare):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size,
                              shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss().to(device)
    ...
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_trn / batch_size))):
            ...
            avg_model_params = torch.cat([param.view(-1) for param in avg_model.parameters()])
            model_params = torch.cat([param.view(-1) for param in model.parameters()])
            loss_momen_reg = torch.norm(model_params - avg_model_params)
            loss += beta * loss_momen_reg
...
def get_layer_parameters(model, name):
    for model_name, param in model.named_parameters():
        if name == model_name:
            return param
    raise ValueError(f"No parameters found for layer with name '{name}'")
...
def train_model_asam(model, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name,
                     args):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size,
                              shuffle=True)
