...
### Methods
def train_FedAvg(data_obj, act_prob ,learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, args, seed, rand_seed=0):
    method_name = 'FedAvg'
    ...
    if args.mu_mean == 0:
        if args.add_sam == 0:
            method_name = 'FedAvg'+str(f"mu_mean:{args.mu_mean}")+"-time"+str(month)+str(day)
        elif args.add_sam == 1 and args.opt_method == 'SAM':
            method_name = 'FedAvg_SAM'+str(f"mu_mean:{args.mu_mean}")+"-time"+str(month)+str(day)
        elif args.add_sam == 1 and args.opt_method == 'ASAM':
            method_name = 'FedAvg_ASAM'+str(f"mu_mean:{args.mu_mean}")+"-time"+str(month)+str(day)
    else:
        method_name = 'FedMAN'+str(f"mu_mean:{args.mu_mean}")+"-time"+str(month)+str(day)
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)): #
        ...
        for j in range(n_save_instances):
            ...
            fed_model = model_func()
            fed_model.load_state_dict(torch.load('Output/%s/%s/%d_com_all.pt' %(data_obj.name, method_name, (j+1)*save_period))) #加载之前训练的all模型
            fed_model.eval()
            fed_model = fed_model.to(device)
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                if args.add_sam == 0:
                    clnt_models[clnt] = train_model(clnt_models[clnt], trn_x, trn_y, learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, data_obj.dataset, args)
                elif args.add_sam == 1 and args.opt_method == 'SAM': 
                    #print("sam")
            ...
            ### dont use saw,save loss and acc of the avgModel in test time at each round
...
def train_FedMA(data_obj, act_prob, learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, args, seed, rand_seed=0):
    current_datetime = datetime.datetime.now()
    month = current_datetime.month
    day = current_datetime.day
    method_name = 'FedMA' + f"-beta{args.beta}"+"-time"+str(month)+str(day)+"-seed"+str(seed)+"norm"
    n_clnt = data_obj.n_client
    ...
    clnt_grads_list = list(range(n_clnt))
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' % (data_obj.name, method_name, com_amount)):  
        ...
        # Start training
        momen_model = model_func().to(device)
        lamda = 0.85 
        for i in range(com_amount):
            print(f"**------{method_name}-----**")
            if i > 0:
                ...
                loss_conpare = False
                for param in momen_model.parameters():
                    param.data.zero_()
            inc_seed = 0
            while (True):
                # Fix randomness in client selection
            ...
            with torch.no_grad():
                avg_pre_model = copy.deepcopy(avg_model)  #t-1
                for name, param in avg_model.named_parameters():
                    momen_param = get_layer_parameters(momen_model, name)
                    param.data.add_(momen_param, alpha=lamda)
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt] = model_func().to(device)
                clnt_models[clnt].load_state_dict(
                    copy.deepcopy(dict(avg_model.named_parameters()))) 

                for params in clnt_models[clnt].parameters():
                    params.requires_grad = True 

                clnt_models[clnt] = train_FedMA_model(clnt_models[clnt], trn_x, trn_y,
                                                                   learning_rate * (lr_decay_per_round ** i),
                                                                   batch_size,
                                                                   epoch, print_per, weight_decay, data_obj.dataset,
                                                                   args,
                                                                   avg_model,
                                                                   momen_model,
                                                                   loss_conpare)
            ...
            all_model = set_client_from_params(model_func(),
                                            np.sum(clnt_params_list * weight_list / np.sum(weight_list),
                                                    axis=0))  # 不管你参与不参与，都进行一次权重缩小

            with torch.no_grad():
                for name, param in momen_model.named_parameters():
                    avg_param = get_layer_parameters(avg_model, name).cpu()
                    avg_pre_param = get_layer_parameters(avg_pre_model, name).cpu()
                    updated_param = avg_param - avg_pre_param
...
def train_FedACG(data_obj, act_prob, learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, args, seed, rand_seed=0):
    current_datetime = datetime.datetime.now()
    month = current_datetime.month
    day = current_datetime.day
    method_name = 'FedACG' +"-time"+str(month)+str(day)+"-seed"+str(seed)
    n_clnt = data_obj.n_client
    clnt_x = data_obj.clnt_x
    clnt_y = data_obj.clnt_y
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' % (data_obj.name, method_name, com_amount)): 
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt] = train_FedACG_model(clnt_models[clnt], trn_x, trn_y,
                                                                   learning_rate * (lr_decay_per_round ** i),
                                                                   batch_size,
                                                                   epoch, print_per, weight_decay, data_obj.dataset,
                                                                   args,
